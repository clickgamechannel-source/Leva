// Race Bot v2 — все функции, чистый код
...
const GROQ = process.env.GROQ_KEY || "gsk_smiYKHRG...";
...
let offset = 0, mem = { facts: [], reminders: [], notes: [], dialogues: [], expenses: [], events: [], habits: {}, shopping: [], newItems: [] };
const ctx = new Map(), voiceOn = new Map(), alarmQ = new Map();
let lastSearch = "", lastMsgId = 0;

const now = () => new Date(new Date().getTime() + 10800000);
const utc = (d) => new Date(d.getTime() - 10800000);
...
async function ai(msg, sys) {
  const r = await fetch(DS, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${DK}` }, body: JSON.stringify({ model: "deepseek-v4-pro", messages: [{ role: "system", content: sys || "Ты Race, ассистент. Отвечай кратко, на русском." }, { role: "user", content: msg }], max_tokens: 500, temperature: 0.7 }) });
...
async function load() { try { const r = await fetch(`${SUP}/rest/v1/bot_memory?id=eq.1&select=data`, { headers: { apikey: SK, Authorization: `Bearer ${SK}` } }); const d = await r.json(); if (d[0]?.data) { const l = d[0].data; mem.facts = l.facts || []; mem.reminders = l.reminders || []; mem.notes = l.notes || []; mem.dialogues = l.dialogues || []; mem.expenses = l.expenses || []; mem.events = l.events || []; mem.habits = l.habits || {}; mem.shopping = l.shopping || []; mem.newItems = l.newItems || []; } } catch {} }
...
function checkRems() { const n = new Date(); mem.reminders = mem.reminders.filter(r => { if (new Date(r.time) <= n) { send(r.chatId || UID, "⏰ Напоминаю: " + r.message).catch(() => {}); return false; } return true; }); }
  ...
  while (true) {
    try {
      ...
      for (const u of d.result || []) {
        ...
        else if (text === "/obsidian" || text.match(/^(?:что в обсидиан|заметк|vault|память|найди в заметк)/i)) { const q = text.replace(/(?:что в обсидиан|заметк|vault|память|найди в заметк|в обсидиане|\/obsidian)/gi, "").trim(); reply = await searchObs(q) || "Ничего не найдено в Obsidian."; }
        else if (text === "/alarm") { alarmQ.set(cid, true); reply = "⏰ На сколько поставить будильник?\nНапиши время, например 7:30"; }
        else if (alarmQ.has(cid)) { alarmQ.delete(cid); const m2 = text.match(/(\d{1,2})[:.](\d{2})/); if (m2) { const h = parseInt(m2[1]), mn = parseInt(m2[2]); if (h >= 0 && h < 24 && mn >= 0 && mn < 60) { const n2 = now(); let t2 = new Date(n2.getFullYear(), n2.getMonth(), n2.getDate(), h, mn, 0); if (t2 <= n2) t2.setDate(t2.getDate() + 1); mem.reminders.push({ time: utc(t2).toISOString(), message: "Будильник", chatId: cid, created: new Date().toISOString() }); await save(); reply = `⏰ Будильник на ${h.toString().padStart(2,"0")}:${mn.toString().padStart(2,"0")}!\n😴 Сладких снов!`; } else { reply = "Время от 0:00 до 23:59. Попробуй ещё раз."; alarmQ.set(cid, true); } } else { reply = "Напиши в формате ЧЧ:ММ, например 7:30"; alarmQ.set(cid, true); } }
        ...
        else if (text.match(/^(?:напомни|таймер)\s+/i)) { const m3 = text.match(/через\s+(\d+)\s+(минут|мин|час|часа|часов|секунд|сек)\s*(.+)?/i) || text.match(/(?:в|на)\s+(\d{1,2})[:.](\d{2})\s*(.+)?/i); if (m3) { const num = parseInt(m3[1]), lbl = m3[3]?.trim() || "напоминание"; const mult = { минут: 60, мин: 60, минуту: 60, минуты: 60, час: 3600, часа: 3600, часов: 3600, секунд: 1, сек: 1 }; const sec = text.includes("в ") || text.includes("на ") ? (() => { const n3 = now(); let t3 = new Date(n3.getFullYear(), n3.getMonth(), n3.getDate(), num, parseInt(m3[2]), 0); if (t3 <= n3) t3.setDate(t3.getDate() + 1); return Math.round((t3 - n3) / 1000); })() : num * (mult[m3[2]] || 60); const t4 = new Date(now().getTime() + sec * 1000); mem.reminders.push({ time: utc(t4).toISOString(), message: lbl, chatId: cid, created: new Date().toISOString() }); await save(); reply = `⏰ Напоминание: ${t4.toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit" })} — «${lbl}»`; } else reply = "Пример: напомни через 10 минут проверить"; }
        else if (text.match(/^(?:мои напоминания|список напоминаний)/i)) { reply = mem.reminders.length ? "Напоминания:\n" + mem.reminders.map((r, i) => `${i + 1}. ${new Date(r.time).toLocaleString("ru-RU")} — ${r.message}`).join("\n") : "Нет напоминаний."; }
        else if (text.match(/^(?:удали напоминание)\s+(\d+)/i)) { const idx = parseInt(text.match(/\d+/)[0]); if (idx > 0 && idx <= mem.reminders.length) { const rm = mem.reminders.splice(idx - 1, 1)[0]; await save(); reply = `Удалено: ${rm.message}`; } else reply = "Неверный номер."; }
        ...
        else if (text.match(/^(?:переведи)\s+(.+)/i)) { const rest = text.replace(/^переведи\s+/i, ""); const lm = rest.match(/^(?:на\s+)?(английский|english|русский|китайский|chinese|немецкий|german|французский|french)/i); if (lm) { const tl = lm[1].match(/англ|english/i) ? "en" : lm[1].match(/кит|chinese/i) ? "zh" : lm[1].match(/нем|german/i) ? "de" : lm[1].match(/фран|french/i) ? "fr" : "ru"; const tt = rest.replace(lm[0], "").trim(); const tr = await fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${tl}&dt=t&q=` + encodeURIComponent(tt)); const td = await tr.json(); reply = td[0]?.map(p => p[0]).join("") || "Не удалось."; } else reply = "На какой язык? Пример: переведи на английский привет"; }
        ...
        else if (text.match(/^(?:добавь встречу|новая встреча)\s+(.+)/i)) { const rest = text.replace(/^(?:добавь встречу|новая встреча)\s+/i, ""); const t5 = new Date().toISOString(); mem.events.push({ date: t5, desc: rest }); await save(); reply = `Встреча добавлена: ${rest}`; }
        ...
        await tg("sendMessage", { chat_id: cid, text: reply });
